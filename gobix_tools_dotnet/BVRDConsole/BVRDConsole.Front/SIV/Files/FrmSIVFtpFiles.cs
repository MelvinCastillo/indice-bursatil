...
{
    public partial class FrmSIVFtpFiles : BVRD.Windows.Controls.Base.FrmBaseDialog
    {
        SIV.BusinessLayer.SIVData _SIVData = new SIV.BusinessLayer.SIVData();

        public FrmSIVFtpFiles()
        {
            InitializeComponent();
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            ...
            {
                string destFile = BVRD.Global.SIV_Parameters.FilePatch + @"\" + filename;

                if (System.IO.File.Exists(destFile))
                {
                    System.IO.File.Delete(destFile);
                }                
                BVRD.FTP.Utils.clFTP.Download(BVRD.Global.SIV_Parameters.FilePatch, filename);
                ...
                if (BVRD.FTP.Utils.clFTP.ftpStatusDescarga.ToString() == "SI")
                {
                    ...
                    {
                        int a = _SIVData.exe_sp_LogOTCSIV(response.LastModified, "Transacciones_BVRD.xml");   
                    }
                }
            }
           /* else
        ...
        {
            lbDefaultFileName.Text = BVRD.Global.SIV_Parameters.GetFileName();
        }       
    }
}
